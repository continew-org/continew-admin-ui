...
const showReLogin = async (failedGeneration: number) => {
...
const retryAfterRefresh = async (config: AxiosRequestConfig, error?: unknown) => {
  ...
  if (retryConfig.__isRetryRequest) {
    await showReLogin(retryConfig.__authGeneration ?? getAuthGeneration())
  ...
  try {
    ...
    // 只有服务端明确判定 Refresh Token 无效时才清理登录态。403、429、5xx
    // 和网络故障都可能是暂时性问题，保留 Cookie 供下一次请求继续重试。
